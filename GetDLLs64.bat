...
XCOPY /Y /D "%QuickGlancePath%\bin64\QuickLinkAPI.dll" "%destdir%"
IF ERRORLEVEL 1 GOTO COPY_FAILED
XCOPY /Y /D "%QuickGlancePath%\bin64\PGRFlyCapture.dll" "%destdir%"
IF ERRORLEVEL 1 GOTO COPY_FAILED

ECHO Your 64-bit DLLs are now up to date.
PAUSE
EXIT /B 0
